...
测试库原本已有邮箱唯一索引，但 `SignupID` 没有唯一约束或数据库默认生成值。本轮新增 `SignupID` 唯一索引和 PostgreSQL sequence，注册接口改为由数据库分配 ID，并保留重复邮箱的原有响应格式。迁移遇到历史重复 ID 会使启动失败，须先核对并处理重复数据；不会静默跳过。

双进程后端在本地端口 8001 最终复测：50 个不同邮箱全部成功且 ID 唯一；同一邮箱 20 次并发请求中 1 次成功、19 次按预期拒绝；不同邮箱请求 p95 为 27.2 ms。51 条测试记录全部清理。脚本还核对了 ID 唯一索引与 sequence 默认值。此结果仅针对本地测试库，未验证线上数据库现存记录。
